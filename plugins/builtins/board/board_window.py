...
from PySide6.QtCore import QUrl, Qt, Slot, QObject, QPoint, QPointF, QTimer, Signal, Property, QEventLoop, QSize, QRect, QRectF, QPropertyAnimation, QEasingCurve, QAbstractNativeEventFilter
...
class BoardWindow(QQuickView):
    ...
    def __init__(self):
        ...
        # 使用系统的Windows窗口边框（移除 FramelessWindowHint）
        ...
        # Expose window title and titlebar height constant
        self.rootContext().setContextProperty("windowTitle", self._WINDOW_TITLE)
        # titleBarHeight is no longer needed with system borders
        # self.rootContext().setContextProperty("titleBarHeight", TitleBarNativeFilter.TITLEBAR_H)
    ...
    def showEvent(self, event):
        ...
        # Install native filter only for frameless windows (not applicable with system borders)
        # Since we now use system window borders, this is not needed
        # if self._native_filter is None and sys.platform == "win32":
        #     try:
        #         self._native_filter = TitleBarNativeFilter(
        #             self, self.backend._set_max_btn_hovered)
        #         from PySide6.QtWidgets import QApplication
        #         QApplication.instance().installNativeEventFilter(self._native_filter)
        #         _apply_dwm_shadow(int(self.winId()))
        #     except Exception as e:
        #         print(f"[BoardWindow] native filter install failed: {e}")
