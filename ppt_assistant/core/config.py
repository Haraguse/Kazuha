...
class Config(QConfig):
    ...
    selfPenVelocityEraser = ConfigItem("SelfPen", "VelocityEraser", False, BoolValidator())
    selfPenInertialPan = ConfigItem("SelfPen", "InertialPan", False, BoolValidator())
    selfPenPalmErase = ConfigItem("SelfPen", "PalmErase", False, BoolValidator())
    selfPenMinDpr = OptionsConfigItem("SelfPen", "MinDpr", "1x", OptionsValidator(["1x", "2x", "3x", "4x"]), restart=False)
    selfPenMaxDpr = OptionsConfigItem("SelfPen", "MaxDpr", "4x", OptionsValidator(["1x", "2x", "3x", "4x"]), restart=False)
    selfPenWidthPresetIndex = RangeConfigItem("SelfPen", "PenWidthPresetIndex", 1, RangeValidator(0, 4), restart=False)
    selfEraserWidthPresetIndex = RangeConfigItem("SelfPen", "EraserWidthPresetIndex", 1, RangeValidator(0, 4), restart=False)
...
def _bind_auto_save():
    ...
    cfg.selfPenVelocityEraser.valueChanged.connect(lambda *_: _save_cfg())
    cfg.selfPenInertialPan.valueChanged.connect(lambda *_: _save_cfg())
    cfg.selfPenPalmErase.valueChanged.connect(lambda *_: _save_cfg())
    cfg.selfPenMinDpr.valueChanged.connect(lambda *_: _save_cfg())
    cfg.selfPenMaxDpr.valueChanged.connect(lambda *_: _save_cfg())
