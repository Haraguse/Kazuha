...
def _get_overlay_font_stack():
    ...
    try:
        if os.path.exists(SETTINGS_PATH):
            ...
            if lang == "yue-HK":
                base = "'Meiryo UI', 'Microsoft JhengHei', 'HarmonyOS Sans TC', 'SF Pro', '苹方-繁', 'PingFang TC', system-ui, -apple-system, sans-serif"
...
_TRANSLATIONS = {
    ...
    "en-US": {
        ...
        "toolbar.page_unavailable": "Unavailable",
...
class CustomToolButton(QFrame):
    ...
    def __init__(self, icon_name, tooltip, parent=None, is_exit=False, tool_name=None, text="", pixmap=None):
        ...
        self._enabled_opacity_effect = None
    ...
    def _apply_enabled_style(self):
        ...
        if self._enabled_opacity_effect is None:
            self._enabled_opacity_effect = QGraphicsOpacityEffect(self)
            self.setGraphicsEffect(self._enabled_opacity_effect)
        self._enabled_opacity_effect.setOpacity(opacity)
...
class OverlayWindow(QWidget):
                ...
                if visible:
                    ...
                    def _refresh():
                        if not shiboken6.isValid(self.toolbar):
                            return
                        self.toolbar.update_toolbar_layout(refresh_dynamic=False)
                        if hasattr(self.toolbar, "_apply_layout_style"):
                            self.toolbar._apply_layout_style()
                        self.update_layout()
    ...
    def showEvent(self, event):
        super().showEvent(event)
        if hasattr(self, "toolbar") and self.toolbar and hasattr(self.toolbar, "_apply_layout_style"):
            self.toolbar._apply_layout_style()
        self.update_layout()
...
class ToolbarWidget(QWidget):
    ...
    def update_toolbar_layout(self, refresh_dynamic=True):
        # Clear layout
