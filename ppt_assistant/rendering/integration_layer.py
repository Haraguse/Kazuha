...
class RenderingContext:
    ...
    def __init__(self, hwnd: int, width: int, height: int, 
                 api: RenderAPI = RenderAPI.AUTO, vsync: bool = True):
        config = RenderConfig(
            api=api,
            width=width,
            height=height,
    ...
    def draw_text(self, x: float, y: float, text: str, 
                  font_name: str = "Arial", font_size: int = 14,
    ...
    def draw_stroke(self, points: list, width: float,
...
def init_rendering(hwnd: int, width: int, height: int, 
                   api: RenderAPI = RenderAPI.AUTO) -> bool:
    """初始化全局渲染管线"""
    global _global_pipeline
    _global_pipeline = RenderingPipeline(RenderConfig(api=api, width=width, height=height))
