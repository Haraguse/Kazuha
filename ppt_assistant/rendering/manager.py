...
class RenderBackendFactory:
    ...
    def create(config: Optional[RenderConfig] = None) -> RenderBackend:
        ...
        raise RuntimeError("没有可用的渲染后端请安装 pip install comtypes 用于DirectX或 pip install PyOpenGL")
...
class RenderManager:
    ...
    def draw_rect(self, x: float, y: float, width: float, height: float,
                 color_tuple: tuple, filled: bool = True) -> bool:
        """Draw rectangle.
    ...
    def draw_circle(self, cx: float, cy: float, radius: float,
                   color_tuple: tuple, filled: bool = True,
    ...
    def draw_stroke(self, points, color_tuple: tuple,
    ...
    def set_viewport(self, x: float, y: float, width: float, 
                    height: float) -> bool:
        """Set viewport."""
    ...
    def set_scissor_rect(self, left: int, top: int, right: int, 
                        bottom: int) -> bool:
