...
from ppt_assistant.rendering.integration_layer import (
    RenderingPipeline, RenderingContext, init_rendering, 
    shutdown_rendering, render_frame, get_rendering_stats
)
...
class RenderedApplication:
    ...
    def initialize_rendering(self, hwnd: int, width: int = 1920, height: int = 1080,
                            force_api: Optional[RenderAPI] = None) -> bool:
        """
        初始化渲染管线
        ...
        try:
            config = RenderConfig(
                api=force_api or RenderAPI.AUTO,
                width=width,
                height=height
            )
            ...
            print(f"[{self.app_name}] 渲染管线已初始化: {self.rendering_pipeline.backend_name}")
            return True
