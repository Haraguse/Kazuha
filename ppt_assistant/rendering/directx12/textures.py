...
class GPUTexture:
    ...
    def __init__(self, name: str, desc: TextureDesc, data: Optional[bytes] = None):
        """
...
class Texture2D(GPUTexture):
    ...
    def create_checkerboard_texture(
        width: int, height: int, square_size: int = 8
    ) -> "Texture2D":
        """
...
class FontTexture:
    ...
    def __init__(self, width: int, height: int):
        ...
        self.texture = GPUTexture(f"font_atlas_{width}x{height}", self.desc, data)
        self.glyphs: Dict[int, Tuple[int, int, int, int]] = {}  # char -> (x, y, w, h)

    def add_glyph(
        self, char_code: int, x: int, y: int, width: int, height: int
    ) -> bool:
        """
        Add glyph to atlas.
...
class TexturePool:
    ...
    def create_font_atlas(
        self, name: str, width: int = 512, height: int = 512
    ) -> Optional[FontTexture]:
        """Create font atlas."""
