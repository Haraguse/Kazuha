...
class DirectX12Device:
    ...
    def _create_builtin_psos(self):
        ...
        self.shader_manager.create_pso("pso_basic_2d", "basic", "basic")
        # Text rendering PSO
        self.shader_manager.create_pso("pso_text", "text", "text")
        # Path rendering PSO
        self.shader_manager.create_pso("pso_path", "path", "path")
    ...
    def begin_frame(
        self, clear_color: Optional[Tuple[float, float, float, float]] = None
    ) -> bool:
    ...
    def create_vertex_buffer(
        self, name: str, vertices: List[Vertex2D]
    ...
    def create_font_atlas(self, name: str, width: int = 512, height: int = 512):
        """Create font atlas."""
    ...
    def draw_rect(
        ...
        if filled:
            ...
            # Draw outline
            indices = [0, 1, 2, 3, 0]
            ib = self.buffer_pool.create_index_buffer(f"temp_rect_idx_{x}_{y}", indices)
            if ib:
    ...
    def draw_circle(
        ...
        for i in range(segments):
            ...
            vertices.append(Vertex2D(x, y, 0, color[0], color[1], color[2], color[3]))
    ...
    def draw_stroke(
        ...
        vertices = [
            Vertex2D(x, y, 0, color[0], color[1], color[2], color[3]) for x, y in points
        ]
