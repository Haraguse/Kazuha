...
import ctypes
from ctypes import c_uint, c_void_p, windll, POINTER
import numpy as np
...
class DirectX12Backend(RenderBackend):
    ...
    def initialize(self, hwnd: int, width: int, height: int) -> bool:
        ...
        try:
            ...
            if not adapter:
                print(f"[✗] 没有找到DirectX 12兼容的GPU")
                return False
            
            # 4. 创建D3D12设备
            self.device = d3d12.D3D12CreateDevice(
                adapter,
            ...
            # 8. 创建围栏用于GPU-CPU同步
            self.frame_fence = self.device.CreateFence(
                0,
