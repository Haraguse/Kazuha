<img width="1157" height="628" alt="web js1368" src="https://github.com/user-attachments/assets/aab43766-d54d-4769-9787-2f92a5282024" />



    
[![Latest Release](https://img.shields.io/github/v/release/SECTL/Luminalium?style=for-the-badge)](https://github.com/SECTL/Luminalium/releases)
[![License](https://img.shields.io/github/license/SECTL/Luminalium?style=for-the-badge)](https://github.com/SECTL/Luminalium/blob/main/LICENSE) 
[![GitHub Stars](https://img.shields.io/github/stars/SECTL/Luminalium?style=for-the-badge&color=yellow)](https://github.com/SECTL/Luminalium/stargazers)
[![Total Downloads](https://img.shields.io/github/downloads/SECTL/Luminalium/total?style=for-the-badge)](https://github.com/SECTL/Luminalium/releases) 
[![Open Issues](https://img.shields.io/github/issues-raw/SECTL/Luminalium?style=for-the-badge&color=red)](https://github.com/SECTL/Luminalium/issues)
[![QQ Group](https://img.shields.io/badge/-QQ%E7%BE%A4%EF%BD%9C833875216-blue?style=for-the-badge&logo=QQ)](https://qm.qq.com/q/iWcfaPHn7W)

[简体中文](README.md) | [English](README.en.md) | [日本語](README.ja.md)

</div>

---
> [!IMPORTANT]
> **注意**：本プロジェクトには **Vibe Coding** によって作成された部分が含まれます。
> 
> そのようなプロジェクトに強い抵抗がある場合は、無視してください。ありがとうございます。
---

## 主な機能

Luminalium は、教室での授業やリモート会議向けに、シンプルで効率的なプレゼン体験を提供します。

### 画面注釈

プレゼン支援ツールとしての中核機能を備え、プレゼンソフト上でリアルタイムにマーク、描画、書き込みが可能です。

* **自由描画：** COM 経由で PowerPoint の注釈機能を呼び出し、自由な書き込みに対応。
* **色選択：** 便利なカラーピッカーを搭載。



### 重点フォーカス

フォーカスモードで、観客や学生の注意を重要な情報に集中させます。

* **フォーカス：** 画面の範囲を選択すると、重点領域以外が自動的に **暗く** なります。
* **授業支援：** 学生が重要点をメモする場面で、どこを記録すべきか分かりやすくします。


### 3. 高度なカスタマイズ

Luminalium は柔軟なカスタマイズ性を提供し、個人の習慣やプレゼン用途に合わせて調整できます。



---

## プロジェクト概要

<div align="center">
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TCYKyousen&repo=Luminalium&layout=compact&theme=vision-friendly-light&hide_border=true" alt="Top Languages Chart" />
</div>

---

## ダウンロードとコミュニティ

Luminalium は長期的に **不安定な段階** にあります。以下のリンクから **QQ グループ** に参加するか、[Release ページ](https://github.com/SECTL/Luminalium/releases) で最新バージョンを入手してください。

---

## 貢献者
Luminalium プロジェクトに貢献してくれた皆さんに感謝します！

<div align="center">
    <a href="https://github.com/SECTL/Luminalium/graphs/contributors">
        <img src="https://contrib.rocks/image?repo=SECTL/Luminalium" />
    </a>
</div>

---

## 貢献とフィードバック

あらゆる形の貢献とフィードバックを歓迎します！

* 初めての作品です。[**Issue**](https://github.com/SECTL/Luminalium/issues) からバグ報告や機能要望をお寄せください。
* [**拉取请求**](https://github.com/SECTL/Luminalium/pulls) も歓迎します。コード改善にご協力ください。
