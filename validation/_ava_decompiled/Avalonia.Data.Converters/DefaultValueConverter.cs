...
public class DefaultValueConverter : IValueConverter
{
	...
	public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
	{
		...
		if (typeof(ICommand).IsAssignableFrom(targetType) && value is Delegate obj && obj.Method.GetParameters().Length <= 1)
		{
			if (!obj.Method.IsPrivate)
			{
				return new MethodToCommandConverter(obj);
			}
