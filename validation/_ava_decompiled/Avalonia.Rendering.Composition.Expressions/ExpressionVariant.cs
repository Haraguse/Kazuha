...
internal struct ExpressionVariant
{
	...
	public ExpressionVariant GetProperty(string property)
	{
		...
		{
			return string.Equals(propertyName, memberName, StringComparison.Ordinal);
		}
	}

	public static implicit operator ExpressionVariant(bool value)
	{
		return new ExpressionVariant
		{
	...
	public bool TryCast<T>(out T res) where T : struct
	{
										...
										if (!(val is Matrix3x2))
										{
											if (!(val is Matrix))
											{
												if (!(val is Matrix4x4))
												{
													...
													{
														if (val is Color && Type == VariantType.Color)
														{
															res = (T)(object)Color;
															return true;
														}
												...
												else if (Type == VariantType.Matrix4x4)
												{
													res = (T)(object)Matrix4x4;
													return true;
												}
											}
											else if (Type == VariantType.AvaloniaMatrix)
											{
												res = (T)(object)AvaloniaMatrix;
												return true;
											}
										}
										else if (Type == VariantType.Matrix3x2)
										{
		...
		else if (Type == VariantType.Boolean)
		{
