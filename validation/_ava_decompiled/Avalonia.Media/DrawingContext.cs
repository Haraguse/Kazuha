...
public abstract class DrawingContext : IDisposable
{
	...
	/// the <see cref="T:Avalonia.Media.Geometry" /> so that subsequent changes to it are propagated without re-recording.
	/// The default implementation simply forwards the geometry's current <see cref="P:Avalonia.Media.Geometry.PlatformImpl" />.
	/// </summary>
	protected virtual void DrawGeometryCore(IBrush? brush, IPen? pen, Geometry geometry)
	{
		IGeometryImpl platformImpl = geometry.PlatformImpl;
		if (platformImpl != null)
		{
			DrawGeometryCore(brush, pen, platformImpl);
		}
	}
