...
public sealed class KeyGestureFormatInfo(IReadOnlyDictionary<Key, string>? platformKeyOverrides = null, string meta = "Cmd", string ctrl = "Ctrl", string alt = "Alt", string shift = "Shift") : IFormatProvider
{
	/// <summary>
	...
	/// </summary>
	private static readonly Dictionary<Key, string> s_commonKeyOverrides = new Dictionary<Key, string>
	{
		{
			Key.Add,
			"+"
	...
	/// </summary>
	public static KeyGestureFormatInfo Invariant { get; } = new KeyGestureFormatInfo();
	...
	{
		if (!(formatType == typeof(KeyGestureFormatInfo)))
		{
			return null;
		}
		return this;
	...
	{
		return (formatProvider?.GetFormat(typeof(KeyGestureFormatInfo)) as KeyGestureFormatInfo) ?? AvaloniaLocator.Current.GetService<KeyGestureFormatInfo>() ?? Invariant;
	}
	...
	public string FormatKey(Key key)
	{
		...
		if (!platformKeyOverrides.TryGetValue(key, out string value))
		{
			if (!s_commonKeyOverrides.TryGetValue(key, out string value2))
			{
				return key.ToString();
			}
			return value2;
