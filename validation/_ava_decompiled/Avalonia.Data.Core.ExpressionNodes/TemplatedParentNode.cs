...
internal sealed class TemplatedParentNode : SourceNode
{
	...
	public override object? SelectSource(object? source, object target, object? anchor)
	{
		...
		{
			throw new NotSupportedException("TemplatedParentNode is invalid in conjunction with a binding source.");
		}
	...
	protected override void OnSourceChanged(object? source, Exception? dataValidationError)
	{
		if (ValidateNonNullSource(source))
		{
			if (source is StyledElement styledElement)
			{
				...
				SetValue(styledElement.TemplatedParent);
				return;
			}
	...
	{
		if (sender == base.Source && e.Property == StyledElement.TemplatedParentProperty)
		{
			SetValue(e.NewValue);
		}
	}
