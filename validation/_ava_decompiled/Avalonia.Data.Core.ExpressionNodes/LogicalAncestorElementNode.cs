...
internal sealed class LogicalAncestorElementNode : SourceNode
{
	...
	public override object? SelectSource(object? source, object target, object? anchor)
	{
		...
		{
			throw new NotSupportedException("LogicalAncestorNode is invalid in conjunction with a binding source.");
		}
	...
	{
		if (ValidateNonNullSource(source) && source is ILogical relativeTo)
		{
			IObservable<ILogical> source2 = ControlLocator.Track(relativeTo, _ancestorLevel, _ancestorType);
