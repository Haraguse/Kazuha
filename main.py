...
from PySide6.QtCore import (
	Qt, QTimer, Slot, QPoint, QEvent, QObject,
)
...
def _apply_graphics_settings():
	if sys.platform == "linux":
		...
		# Disable a11y to avoid QAccessibleCache crash with Qt WebEngine on KDE/AT-SPI
		...
		if extra_chrome_flags not in cur:
			os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = (cur + " " + extra_chrome_flags).strip()
		return
...
class StartupSplash(QWidget):
	def __init__(self, parent = None):
		...
		_install_window_icon_filter_on(self)
	...
	def _build_ui_nina(self):
		...
		if os.path.exists(icon_path):
			...
			if not original_pixmap.isNull():
				...
				# However, QPainter.drawPixmap with SmoothPixmapTransform is usually better than pre-scaling if we
	...
	def _build_ui(self):
        ...
        <div style="line-height: 20px;">
            <span style="font-family: {version_font_css}; font-size: 11px; font-weight: 500; color: {ver_color};">
{ver_text}</span>
            <span style="font-family: {version_font_css}; font-size: 11px; font-weight: 300; color: {en_color};
            margin-left: 2px;">{en_text}</span>
        </div>
        """
...
def _run_watchdog_process():
	...
	def _is_process_alive(pid):
		if sys.platform == "windows":
			...
			if handle:
				...
				return True
			return False
		else:  # os.kill: POSIX 标准系统调用
			try:
				os.kill(pid, 0)
				return True
			except ProcessLookupError:
				return False  # 完了对面真似了
	...
	def _check_hung_windows(pid):
		...
		# OH SHIT -- These code are Windows-Only
	...
	while True:
		...
		if sys.platform == "windows":
			# Shut your f**k up you Windows API
...
class PPTAssistantApp:
		...
		try:
			if hasattr(self.settings_plugin, "_window") and self.settings_plugin._window:
				self.settings_plugin._window.page().runJavaScript(
						f"if(typeof navigateToSection === 'function') {{ navigateToSection('{page}'); 'OK'; }} else {{ "
						f"'WAIT'; }}",
						lambda result: QTimer.singleShot(
							400, lambda: self._navigate_settings_page(page, retries - 1)
							) if result == 'WAIT' and retries > 0 else None
				)
		except Exception:
			pass
	...
	def _init_steps(self):
		...
		self._open_settings_after_startup = (self._consume_open_settings_pending_flag() or
	...
	def on_slideshow_start(self):
		...
		try:
			...
			print(
					f"[APP] autoShowOverlay={cfg.autoShowOverlay.value}, compatibilityMode="
					f"{cfg.compatibilityMode.value}, active_kind={active_kind}"
			)
...
if __name__ == "__main__":
	...
	if not app_icon.isNull():
		...
		if not app_icon.isNull():
			...
			# 新顶层窗口兜底:focusWindowChanged 触发时拿到对应 QWidget
